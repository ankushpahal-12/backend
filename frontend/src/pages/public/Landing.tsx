...
import RealTimeLoader from '../../components/landing/RealTimeLoader';
...
const Landing: React.FC = () => {
  const {
    ...
    showLoader,
    hideLoader,
  } = useRealTimeLoader('landing-loader', true);

  // Listen for website-wide processing status updates
  useEffect(() => {
    const handleWebsiteStatus = (statusData: any) => {
      if (statusData.isProcessing) {
        showLoader(
          statusData.message || 'Website is processing...',
          statusData.progress || 0,
          statusData.status || 'processing',
          statusData.subMessage
        );
      } else {
        hideLoader();
      }
    ...
    socketService.onWebsiteStatus(handleWebsiteStatus);

    return () => {
      socketService.offWebsiteStatus();
    };
  ...
  return (
    <>
      <main className="relative w-full min-h-screen bg-white dark:bg-slate-950 overflow-x-hidden text-slate-900 dark:text-slate-50 font-sans selection:bg-purple-500/30 transition-colors duration-300">
        ...
        {/* Dynamic Background Particle Connection Canvas */}
        <DynamicBackground className="opacity-40 dark:opacity-60" />
